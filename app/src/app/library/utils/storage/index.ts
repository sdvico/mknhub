...
export async function loadString(key: string, option?: MMKVOption) {
  try {
    return await MMKVStorage.getString(key, StyleSheet.flatten([optionMMKV, option]));
  } catch {
 ...
 */
export async function saveString(key: string, value: string, option?: MMKVOption) {
  try {
    await MMKVStorage.setString(key, value, StyleSheet.flatten([optionMMKV, option]));
    return true;
  } catch {
    return false;
  }
...
export async function load(key: string, option?: MMKVOption) {
  try {
    const almostThere = await MMKVStorage.getString(key, StyleSheet.flatten([optionMMKV, option]));
...
export async function save(key: string, value: any, option?: MMKVOption) {
  try {
    await MMKVStorage.setString(key, JSON.stringify(value), StyleSheet.flatten([optionMMKV, option]));
    return true;
  } catch {
    return false;
  }
