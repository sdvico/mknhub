...
const OtpComponent = (props: OtpProps) => {
  ...
  const wrapInput = useMemo(() => enhance([styles.otpView, wrapInputStyle]), [wrapInputStyle]);
  const wrapInputActive = useMemo(() => enhance([styles.otpViewActive, wrapInputActiveStyle]), [wrapInputActiveStyle]);
              ...
              return (
                <View key={index} style={row}>
                  <View
                    style={[
                      wrapInput,
                      (index === otp.length || (length === otp.length && index === otp.length - 1)) &&
                        isFocused &&
                        wrapInputActive,
                    ]}>
                    <Text
                      children={index <= otp.length - 1 ? textEntry?.charAt(0) ?? otp.charAt(index) : ''}
                      style={[text]}
                    />
                  </View>
                  <Spacer width={15} />
