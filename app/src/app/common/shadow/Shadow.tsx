...
import { I18nManager, StyleSheet, View, PixelRatio, Platform } from 'react-native';
import { Defs, LinearGradient, Mask, Path, Rect, Stop, Svg, RadialGradient } from 'react-native-svg';
...
export const sumDps: (a: number, b: number) => number = isWeb ? (a, b) => a + b : (a, b) => R((P(a) + P(b)) / scale);

/** Converts two sizes to pixel for perfect math, divides them and converts the result back to dp. */
export const divDps: (a: number, b: number) => number = isWeb ? (a, b) => a / b : (a, b) => P(a) / P(b);
...
function ShadowInner(props: ShadowProps): JSX.Element {
  ...
  const childProps: { style?: ViewStyle; s?: ViewStyle } =
    Children.count(children) === 1 ? (Children.only(children) as JSX.Element).props ?? emptyObj : emptyObj;
...
function getShadow({
  ...
  const sides = distance > 0 && (
    <>
      ...
      {activeSides.start && (typeof height === 'number' ? height > topStart + bottomStart : true) && (
        <Svg
      ...
      {activeSides.bottom && (typeof width === 'number' ? width > bottomStart + bottomEnd : true) && (
        <Svg
          width={widthWithAdditional}
          height={distanceWithAdditional}
          style={{
    ...
    <Svg
      ...
      {typeof width === 'number' && typeof height === 'number' ? (
        ...
        <Path
          ...
          } v${-bottomEnd} h${bottomEnd} v${-height + bottomEnd + topEnd} h${-topEnd} v${-topEnd} h${
  ...
  return (
    ...
    <View style={containerStyle} pointerEvents="box-none" {...containerViewProps}>
      <View
        pointerEvents="none"
        {...shadowViewProps}
        style={[rtlAbsoluteFillObject, shadowViewProps?.style, { start: offset[0], top: offset[1] }]}>
        {shadow}
      </View>
      <View
